...
public class UserUtility {
	...
	HashMap<String, User> userdeatils = new HashMap<String, User>();
...
// }
	...
	public void register() {
		...
		User customer = new User(name, email, mobilenumber, password, "customer");
	...
	public void login() throws CloneNotSupportedException {
		...
		String name = sc.nextLine();
		String password = sc.nextLine();
		User user = userdeatils.get(name);
		if (user == null) {
			System.out.println("User does not exist");
		} else if (user.password.equals(PasswordUtility.encrpte(password))) {
	...
	private void ManagerActivity(User user) {
		while (true) {
			System.out.println(
					" 1 - change password \n 2 - Update profile \n 3 - Add inventory \n 4 -Update inventory \n 5 - Remove inventory \n 6 - View inventory \n 7 - Exit ");
	...
	private void AdminActivity(User user) {
		while (true) {
			System.out.println(
					" 1 - Change password \n 2 - Update profile \n 3 - Create Manager \n 4 -Update Manager \n 5 - Remove Manager \n 6 - Add inventory \n 7 Update inventory \n 8 - Remove inventory \n 9 - View inventory \n 10 - Exit ");
			int choice = sc.nextInt();
	...
	private void CustomerActivity(User user) throws CloneNotSupportedException {
		boolean exit = false;
		while (!exit) {
			System.out.println(
	...
	private void redeemRewardPoints(User customer) {
	...
	private void viewBalance(User customer) {
		System.out.println(CustomerCart.get(customer.name).currentBalance);
	}

	private void addMoneyToWallet(User customer) {
	...
	private void viewPurchaseHistory(User customer) {
	...
	private void purchase(User customer) throws CloneNotSupportedException {
		// HashMap<String,List<Cart>>CustomerPurchaseHistory = new HashMap<>();
	...
	private void removeFromCart(User customer) {
	...
	private void addToCart(User customer) {
		...
		if (inventary != null) {
			if (inventary.productquantity < quantity) {
				...
				if (cart == null) {
					cart = new Cart();
					Inventary forcustomerinventary = new Inventary(productname, inventary.productprice * quantity,
							quantity);
	...
	private void addInventory(User user) {
		...
		Inventary inventary = new Inventary(productname, productprice, productquantity, description, user.name);
	...
	private void removeManager() {
		...
		String name = sc.nextLine();
		User customer = userdeatils.get(name);
		if (customer != null) {
	...
	private void createManager() {
		...
		String password = sc.nextLine();
		String mobilenumber = sc.nextLine();
		User customer = new User(name, email, mobilenumber, password, "manager");
		userdeatils.put(name, customer);
		System.out.println("Manager created successfully");
	}

	private void updateProfile(User customer) {
	...
	private void changePassword(User customer) {
		System.out.println("Enter new password:");
		String password = sc.nextLine();
		customer.setPassword(PasswordUtility.encrpte(password));
		customer.setUpdatedTime(PasswordUtility.timeGenerate());
